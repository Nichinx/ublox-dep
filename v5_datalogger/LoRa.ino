...
void key_gen(char *_payload_received) {
  ...
  if (get_logger_mode() == 2) {
    ...
    // key_ptr = strncat(ack_msg, ack_key, 7);
    strncat(ack_msg, _payload_received, 5); // v1 - 4
    strncat(ack_msg, ack_key, 7);
  }
  // ack_msg[12] = '\0'; // v1
  ack_msg[13] = '\0';
...
void receive_lora_data(uint8_t mode) {
  ...
  while (rcv_LoRa_flag == 0) {
    uint8_t buf_rcv[RH_RF95_MAX_MESSAGE_LEN];
    uint8_t len_rcv = sizeof(buf_rcv);

    if (rf95.available()) {
      // Should be a message for us now
      if (rf95.recv(buf_rcv, &len_rcv)) {
        int i = 0;
        // len2 = sizeof(buf);
        for (i = 0; i < len_rcv; ++i) {
          received[i] = (uint8_t)buf_rcv[i];
        }
        received[i] = (uint8_t)'\0';
        ...
        if (strstr(received, ">>")) { /*NOT LoRa: 0, 2, 6, 7*/
          ...
          if (mode == 1 || mode == 3 || mode == 4 || mode == 5 || mode == 7) {  // LoRa mode only
...
/////UBX RELATED
//receive ublox data
void receive_ublox_data(uint8_t mode) {
      ...
      if (rf95.recv(buf_rcv, &len_rcv)) {
        ...
        if (strstr(received, "SINUA")) { 
          ...
          if (mode == 1 || mode == 3 || mode == 4 || mode == 5 || mode == 7) {
            if (logger_ack_filter_enabled()) {
              Serial.println("logger_ack_filter_enabled()");
              if (if_receive_valid(received)) {
                Serial.println("------------------if_ubx_receive_valid");
                lora_TX_end++;
                if (mode == 4) {  // 2 LoRa transmitter
                  count2++;
                  Serial.print("recieved counter: ");
...
bool if_receive_valid(char *_received) {
  ...
  _Blog = get_logger_B_from_flashMem();
  _Clog = get_logger_C_from_flashMem();
  _Dlog = get_logger_D_from_flashMem();
  // _Blog[4] = '\0';
  // _Clog[4] = '\0';
  // _Dlog[4] = '\0';
  _Blog[5] = '\0';
  ...
  // snprintf(get_logger_B_from_flashMem(),4,)

  // Serial.print(_Blog); Serial.println(">>>>>");
  // Serial.println(strstr(_received,_Blog));
  if (strstr(ack_msg,_Blog)!=NULL) {
    ...
    // Serial.print("\t");
    // Serial.print("\t"); Serial.print(_Clog); 
    // Serial.print("\t"); Serial.println(_Dlog);
  }
